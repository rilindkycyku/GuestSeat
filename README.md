...
- **Share & QR**: share the whole event via a link (no server — the state travels in the URL), or show a **QR code** guests can scan. Links use a compact, index-based encoding of the seating state, which is roughly 3× smaller than the raw JSON — enough to fit a ~200-guest list into a single QR code. The seating PDF carries this share QR; lists that still exceed a QR's capacity fall back automatically to sending the link — via the native share sheet, a one-tap WhatsApp button, or copy-to-clipboard.
