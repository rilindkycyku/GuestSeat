...
export function iconForAgenda(item: AgendaItem): IconKind {
  ...
  // More Albanian traditions — matched ahead of the generic bride/dinner/music glyphs so the bride's
  // send-off (flamuri), the krushq feast (sofra) and a çifteli folk band each carry their own mark
  // rather than collapsing into the bride figure, a plate, or a note. ("popullore" stays with the
  // circle-dance glyph above, so it isn't repeated here.)
  if (has('marrj', 'përcjell', 'percjell', 'flamur', 'send-off', 'sendoff', 'send off')) return 'flag';
  if (has('çifteli', 'cifteli', 'sharki', 'folk', 'folklor', 'ansambël', 'ansambel')) return 'cifteli';
