...
export function ExportMenu({ state }: { state: EventState }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-48 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg overflow-hidden z-20">
          <button
            onClick={() => {
              exportAsJson(state);
              setOpen(false);
          ...
          >
            JSON <span className="text-slate-400 text-xs block">Full seating data</span>
          </button>
          <button
            onClick={() => {
              exportAsPdf(state);
          ...
          >
            PDF <span className="text-slate-400 text-xs block">Printable seating chart</span>
          </button>
          <button
            onClick={() => {
              exportAsCsv(state);
            ...
            className="w-full text-left px-4 py-2.5 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 border-t border-slate-100 dark:border-slate-800"
          >
            CSV <span className="text-slate-400 text-xs block">Spreadsheet-friendly</span>
          </button>
        </div>
      )}
    </div>
